...
test_case(){
  ...
  echo "$(tput setaf 6)Testing easena compilation of $easea_cflag $directory_name $(tput sgr 0)"
  easena $easea_cflag $program_name.ez > /dev/null;
  error_code=$?;
  if [[ $error_code != 0 ]] ; then
    echo "$(tput setaf 1)Error, easena compilation case $easea_cflag $directory_name $(tput sgr 0)"
    clean $program_name;
    #exit 1;
  else 
