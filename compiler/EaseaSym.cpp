...
///*
    ...
    if (strcmp(sName,"Genome")) {
        ...
        // creation of operator !=
        fprintf(fp,
                "  bool operator!=(%s &EASEA_Var) const {return !(*this==EASEA_Var);} // operator!=\n\n",
                sName); // operator!=
        ...
        while ((pSym=pSymbolList->walkToNextItem())) {
            ...
            if (pSym->Object->ObjectType==oPointer)
                fprintf(fp,
                        "    if (EASEA_Var.%s) os << \"%s:\" << *(EASEA_Var.%s) << \"\\n\";\n",
                        pSym->Object->sName,pSym->Object->sName,pSym->Object->sName);
        }
        fprintf(fp,
...
void CSymbol::printClasses(FILE *fp) {
    ...
    while ((pSym=pSymbolList->walkToNextItem()))
        if ((pSym->Object->pType->ObjectType==oUserClass)
                &&(!pSym->Object->pType->bAlreadyPrinted))
...
void CSymbol::printUC(FILE* fp) {
    ...
    if (strcmp(sName,"Genome") && (TARGET==CUDA || TARGET==STD
                                   || TARGET==NSGA2)) { // If we are printing a user class other than the genome
        fprintf(fp,"\nclass %s;\n",sName); // class  header
    }
    //DEBUG_PRT("%s",sName);
    ...
    while((pSym=pSymbolList->walkToNextItem())) {
        ...
        if(pSym->Object->pType->ObjectType==oUserClass) {
            ...
            if (pSym->Object->ObjectType==oObject) {
                ...
                fprintf(fpOutputFile,"\t\tEASEA_Line << this->%s[EASEA_Ndx] <<\" \";\n",
                        pSym->Object->sName);
            } else if(pSym->Object->ObjectType==oPointer
                      && strcmp(pSym->Object->pType->sName, "GPNode") == 0) {
...
void CSymbol::printAllSymbols(FILE *fp, char *sCompleteName,
                              EObjectType FatherType, CListItem<CSymbol *> *pSym) {
    ...
    do {
        if (pSym->Object->pType->ObjectType==oUserClass) {
            ...
            if (pSym->Object->pType==pSym->Object->pClass)
                fprintf(fp,"%s\n",sNewCompleteName);
            else printAllSymbols(fp, sNewCompleteName, pSym->Object->ObjectType,
                                     pSym->Object->pType->pSymbolList->getHead());
            strcpy(sNewCompleteName, sCompleteName);
        } else {
            if (FatherType==oPointer)
                strcat(sNewCompleteName,"->");
