...
template <class T> class CLList {
  ...
  public :
    ...
    void addFirst(const T &NewObject) {
        pHead = new CListItem<T>(pHead, NewObject);
    }
    void reset() {
        pCurrentObject=pNextItem=pHead;
    }
    CListItem<T> * getHead() {
        return pHead;
    }
    T CurrentObject() {
        return pCurrentObject?pCurrentObject->Object:NULL;
    }
    T nextObject() {
        return pNextItem ?pNextItem->Object:NULL;
    }
...
// Attributes
  public:
    ...
    CLList<CSymbol *>
    *pSymbolList; // pointer on a list of class members (if the symbol is a class)
