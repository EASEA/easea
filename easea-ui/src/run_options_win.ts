...
export class Run_options {
    ...
    constructor() {
        ...
        const time_limit = new Advanced_option_widget('Time Limit (in minutes) :', 2);
        ...
        select_op.text_edit.addEventListener('textChanged', () => {
            var text = select_op.text_edit.text().toLowerCase();
        ...
        reduce_final_op.text_edit.addEventListener('textChanged', () => {
            var text = reduce_final_op.text_edit.text().toLowerCase();
            if (text === 'tournament' || text === '') {
        ...
        global_layout.addWidget(select_op.widget, 3, 0);
        global_layout.addWidget(select_pressure.widget, 3, 1);
        global_layout.addWidget(reduce_final_op.widget, 3, 2);
