...
export class Run_tab {
        ...
        this.run_btn.addEventListener('clicked', () => {
            ...
            if (this.ez_file_address) {
                ...
                if (this.batch_size > 0) {
                    ...
                    fs_extra.emptyDirSync('/tmp/plotting');
                    ...
                    if(this.island_obj.local){
                        ...
                        for(let i = 1; i <= nb_files; i++){
                            let file_name = '/tmp/plotting/ip_file' + i + '.txt'
                    ...
                    for (let i = 0; i < this.batch_size; i++) {
                        ...
                        if(!this.activate_island_model.isChecked()){
                            ...
                            if(i === 0){
                                var run = output_run.run(cmd, this.nb_plots, true, this.options, this.dir_path, i + 1);
                            } else {
                                var run = output_run.run(cmd, this.nb_plots, false, this.options, this.dir_path, i + 1);
                            }
                            ...
                            for(let k = 0; k < nb_files; k++){
                                if(this.island_run_array[k][0][0] < this.batch_size){
                                    ...
                                    for(let l = 0; l < this.island_run_array[k].length; l++){
                                        if(l === 0 && k === 0){
                                            this.run_local_islands(k, l, output_run, this.island_run_array[k][l][3], nb_files, true);
                                        } else {
                                            this.run_local_islands(k, l, output_run, this.island_run_array[k][l][3], nb_files, false);
                                        }
                                    }
                                }
                            }
    ...
    run_local_islands(island_group: number, island_num: number, output_run: Pseudo_term, rank: number, nb_distributed_island: number, print: boolean) {
        ...
        this.options = this.options.concat(' --ipFile /tmp/plotting/ip_file' + (this.island_run_array[island_group][island_num][1] + 1) + '.txt');
        
        var local_run = output_run.run(cmd, this.nb_plots, print, this.options, this.dir_path, (rank + 1));
                ...
                if(next_run < this.batch_size){    
                    if(ok){
                        ...
                        for(let i = 0; i < this.island_run_array[island_group].length; i++){
                            let rank = this.island_run_array[island_group][i][3];
                            if(i === 0){
                                print = true;
                            } else {
                                print = false;
                            }
                            this.run_local_islands(island_group, i, output_run, i+(next_run*this.island_obj.nb_isl_per_run), nb_distributed_island, print);
    ...
    build_run_options(rank: number) {
        ...
        if (this.option_obj.select_op && this.option_obj.select_op.toLowerCase() !== 'tournament')
        ...
        if (this.option_obj.reduce_final_op && this.option_obj.reduce_final_op.toLowerCase() !== 'tournament')
        ...
        if (this.off_obj.reduce_op && this.off_obj.reduce_op.toLowerCase() !== 'tournament')
        ...
        if (this.parent_obj.reduce_op && this.parent_obj.reduce_op.toLowerCase() !== 'tournament')
