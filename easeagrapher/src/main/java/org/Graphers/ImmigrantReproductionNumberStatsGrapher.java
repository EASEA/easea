...
public class ImmigrantReproductionNumberStatsGrapher extends JPanel {
    final int PAD = 30;
    private Frame frame;
    ...
    public ImmigrantReproductionNumberStatsGrapher(Frame f) {
        this.frame = f;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        int maxValue = (int)this.frame.getConsole().getData().getMaximumNumberImmigrantReproductionValue();
        if (maxValue % 5 == 0) {
            maxValue += 5;
        } else {
        ...
        String legend = "Nb of Immigrant Reproductions";
        ...
        double y;
        double x;
        for(i = 0; i < this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().size() - 1; ++i) {
            x = 30.0 + (double)((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).getNbGen() * this.xInc;
            y = (double)(h - 30) - this.scale * ((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).getValue();
            double x2 = 30.0 + (double)((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i + 1)).getNbGen() * this.xInc;
            double y2 = (double)(h - 30) - this.scale * ((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i + 1)).getValue();
            this.g2.draw(new Line2D.Double(x, y, x2, y2));
        }

        this.g2.setPaint(Color.red.darker());

        for(i = 0; i < this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().size(); ++i) {
            x = 30.0 + (double)((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).getNbGen() * this.xInc;
            y = (double)(h - 30) - ((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).getValue() * this.scale;
            this.g2.fill(new Ellipse2D.Double(x - 1.0, y - 1.0, 2.0, 2.0));
            ((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).setX(x);
            ((IslandStatCoordinate)this.frame.getConsole().getData().getNumberOfImmigrantReproductionList().get(i)).setY(y);
        }
