\TEMPLATE_START
/**
 This is program entry for FastEMO template for EASEA
 
 FastEMO for multi-objective optimization.
...
int main(int argc, char** argv){
	...
	LOG_MSG(msgType::INFO, "FastEMO starting....");
    try{
...
//      LOG_MSG(msgType::INFO, msg.str());
    ...
    LOG_MSG(msgType::INFO, "FastEMO successfully completed.");
...
#include "CBestSelection.h"
#include "CBLXCrossover.h"
#include "CGaussianMutation.h"
...
IndividualImpl::IndividualImpl(const IndividualImpl *genome): CIndividual(){ 
...
void EvolutionaryAlgorithmImpl::runEvolutionaryMultiObjectivesLoop(){
  ...
  int archivePopulationSize   = 15*getNumberOfObjectives(); //*(int *) getInputParameter("archivePopulationSize");
  ...
  auto crossoverOperator = make_unique<CBLXCrossover<IndividualImpl>>(xoverProbability, 0.75); //xoverDistributionId, 0.33);

  double mutationProbability = getMutationProbability();
  auto mutationOperator = make_unique<CGaussianMutation<IndividualImpl>>(mutationProbability,
 0.5);// mutationDistributionId);
    
  auto selectionOperator = make_unique<CBTSelection<IndividualImpl,PopulationImpl>>();
  auto selectBestOperator = make_unique<CBestSelection<IndividualImpl,PopulationImpl>>();
  ...
  while (evaluations < maxEvaluations) {
		...
		if (i < 2)
		    parents[1] = selectionOperator->run(population);
        	    ...
        	    parents[1] = selectBestOperator->run(offspringPopulation);
...
if (evaluations < maxEvaluations){
 ...
 PopulationImpl * result = new PopulationImpl(archivePopulation->size()); //+front->size());//ranking->getSubfront(0)->size());
  for (size_t i=0;i<archivePopulation->size();i++) {
...
IndividualImpl * PopulationImpl::get(size_t ind) const{
...
public:
	IndividualImpl();
	IndividualImpl(const IndividualImpl *genome);
   ~IndividualImpl();
	float evaluate();
...
public:
    ...
    IndividualImpl *get(size_t index)const;
