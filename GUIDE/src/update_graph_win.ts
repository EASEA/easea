...
import { Direction, QBoxLayout, QCheckBox, QColorDialog, QDialog, QGridLayout, QLabel, QPushButton, QWidget, WindowType } from "@nodegui/nodegui";
...
export class Update_graph_win {
    ...
    constructor() {
        ...
        axe_x_box.text_edit.addEventListener('textChanged', () => {
            ...
            if (!text && run_obj.plot_type == "2D") {
                this.x_axis = 'Generations';
            }else if(!text){
                this.x_axis = 'f1';
            } else {
                this.x_axis = text;
        ...
        save_btn.addEventListener('clicked', () => {
            ...
            this.last_only = this.check_gen.isChecked();
            run_obj.plot_last_gen_only = this.check_gen.isChecked();
        ...
        if(run_obj.plot_type === "2D")
            layout.addWidget(this.check_gen,5,0);
        
        layout.addWidget(btn_widget,7,0);
        layout.addWidget(this.color_palet,6,0);
    ...
    execution() {
        ...
        if (this.changed) {
            ...
            plot_obj.update_plot(
                run_obj.dir_path + paths.dir_tmp_path + 'fig.svg', 
                this.nb_plots, 
                run_obj.plot_type,
