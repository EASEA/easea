...
export class Pseudo_term {
    ...
    run(cmd: string, plot_size: number, print:boolean, file: string, params?: string, dir?: string, rank?: number) {
        ...
        child.stdout.on('data', (data: string) => {
            ...
            if(rank){
                plot_generation.parser(data, rank);
            } else {
                plot_generation.parser(data, -1);
            }
            ...
            // multi objectives
                ...
                for (let i = 0; i < array.length - 2; i++) {
                    if (array[i].startsWith('Total execution time') ||
                        ...
                        write_in_file(array[i] + '\n', file, run_obj.batch_id.toString(), rank);
                    }
                }
            }
        });
        ...
        child.on('exit', (code, signal) => {
            ...
            if(run_obj.island_model && run_obj.island_obj.local){
                ...
                plot_obj.update_plot(run_obj.dir_path + paths.svg_graph_path, plot_size, run_obj.plot_type, run_obj.dir_path + 'objectives', '', '', '', '', run_obj.plot_last_gen_only);
