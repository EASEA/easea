...
export class Plot_result {
    ...
    update_plot(path: string, plot_size: number, type: string, csv_path: string, title: string, x_name: string, y_name: string, z_name: string, last_only: boolean = true, color?: string) {
        ...
        if (type === '2D') {
            if(color) {
                run = spawn('python3', [plot_path, run_obj.dir_path + paths.dir_tmp_path, nb_gen.toString(), plot_size.toString(), title, x_name, y_name, String(last_only), color], { timeout: 20000 });
            } else {
