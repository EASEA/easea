...
CSelectionOperator* getSelectionOperator(std::string selectop, int minimizing,
    ...
    if(minimizing) {
        ...
        else if(selectop.compare("Identity")==0)
            return (new Identity(globalRandomGenerator));
        else {
            std::cout <<
                      "Operateur n\'existe pas pour minimisation, utilise Tournament par defaut" <<
                      std::endl;
            return (new MinTournament(globalRandomGenerator));
        }
    } else {
        if(selectop.compare("Tournament")==0)
            ...
            std::cout <<
                      "Operateur n\'existe pas pour maximisation, utilise Tournament par defaut" <<
                      std::endl;
            return (new MaxTournament(globalRandomGenerator));
        }
    }
...
****************************************/
void CSelectionOperator::initialize(CIndividual** population,
                                    float selectionPressure, size_t populationSize) {
    this->population = population;
    this->currentSelectionPressure = selectionPressure;
}

size_t CSelectionOperator::selectNext(size_t populationSize) {
    return 0;
}


void Identity::initialize(CIndividual** population, float selectionPressure,
...
size_t Identity::selectNext(size_t populationSize) {
    int next = (count + 1) % populationSize;
    return next;
}

float Identity::getExtremum() {
    return -FLT_MAX;
}
...
****************************************/
void MaxDeterministic::initialize(CIndividual** population,
                                  float selectionPressure,size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
    CPopulation::sortPopulation(population,populationSize);
    this->populationSize = populationSize;
...
****************************************/
void MinDeterministic::initialize(CIndividual** population,
                                  float selectionPressure,size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
...
void MaxRandom::initialize(CIndividual** population, float selectionPressure,
                           size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
}
...
void MinRandom::initialize(CIndividual** population, float selectionPressure,
                           size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
}
...
/* ****************************************
   MinTournament class
****************************************/
void MinTournament::initialize(CIndividual** population,
                               float selectionPressure, size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
}
...
size_t MinTournament::selectNext(size_t populationSize) {
    ...
    if( currentSelectionPressure >= 2 ) {
        ...
        std::cerr <<
                  " MinTournament selection operator doesn't handle selection pressure : "
...
/* ****************************************
   MaxTournament class
****************************************/
void MaxTournament::initialize(CIndividual** population,
                               float selectionPressure, size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
}
 ...
 *    MaxRoulette class
 *****************************************/
void MaxRoulette::initialize(CIndividual** population, float selectionPressure,
                             size_t populationSize) {
    CSelectionOperator::initialize(population,selectionPressure,populationSize);
    CPopulation::sortRPopulation(population,populationSize);
    this->populationSize = populationSize;
