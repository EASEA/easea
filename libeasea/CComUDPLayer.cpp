...
void * CComUDPServer::UDP_server_thread(void *parm) {
    ...
    for(;;) {/*forever loop*/
        ...
        if ((recvMsgSize = recvfrom(p->Socket,buffer,MAXINDSIZE,0,
...
CComUDPServer::CComUDPServer(unsigned short port,int dg) {
    ...
    this->parm = (UDP_server_thread_parm_t*)malloc(sizeof(
    ...
    if(pthread_create(&thread, NULL, &CComUDPServer::UDP_server_thread,
                      (void *)this->parm) != 0) {
...
void CComUDPClient::CComUDP_client_send(char *individual) {
    ...
    int sendbuff=35000;
#ifdef WIN32
    setsockopt(this->Socket, SOL_SOCKET, SO_SNDBUF, (char*)&sendbuff,
               sizeof(sendbuff));
#else
    ...
    if(strlen(individual) < (unsigned)sendbuff ) {
        int n_sent = sendto(this->Socket,individual,strlen(individual),0,
        ...
        fprintf(stderr,"Not sending individual with strlen(): %i, MAX msg size %i\n",
...
bool checkValidLine(string line) {
    ...
    if(address==NULL || port==NULL) {
        cout << "*** WARNING ***\nThere is a problem with the following IP: " << line <<
             "\t===> IGNORING IT\n";
        return false;
    }
...
struct sockaddr_in parse_addr_string(const char* line) {
    ...
    addr.sin_addr.s_addr = inet_addr(
                               res_field_ptr);     /* Any incoming interface */
...
#ifndef WIN32
            if(!isLocalMachine(inet_ntoa(tmpClient.sin_addr), ntohs(tmpClient.sin_port),
                               portServer)) {
