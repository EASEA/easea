/*
 *    Copyright (C) 2009,2013  Ogier Maitre, Waldo Cancino
...
int CComFileServer::refresh_file_list()
{
  ...
  {  
       printf ("(incorrect) Cannot scan the experiment directory for find workers\n");
       return -1;
  }      
  return 0;
}
...
int CComCloudFileServer::refresh_file_list()
{
  // clear the file to process
  new_files.clear();
...
int CComCloudFileServer::determine_worker_name(int start)
{
...
CComCloudFileServer::CComCloudFileServer(char* path, char* expname, std::queue< string >* _data, int dbg) {
    ...
    // now create thread to listen for incoming files
    if(pthread_create(&thread, NULL, &CComCloudFileServer::File_server_thread, (void *)this) != 0) {
        printf("pthread create failed. exiting\n"); exit(1);
    }
}
...
int CComCloudFileServer::determine_file_name(std::string fulltmpfilename, int dest)
{
...
int CComCloudFileServer::create_tmp_file(int &fd, int dest, std::string &fullfilename)
{
   time_t tstamp = time(NULL);
    while(1)
    {
...
int CComCloudFileServer::file_read(const char *filename)
{
    std::string workerfile(filename);
    std::string fullfilename = fullpath + '/' + workername + '/' + workerfile;
    int fd = gfal_open( fullfilename.c_str(), O_RDONLY, 0777);;
...
void * CComCloudFileServer::File_server_thread(void *parm) {
	CComCloudFileServer *server = (CComCloudFileServer*)parm;
	server->run();
	return NULL;
}	

int CComCloudFileServer::number_of_workers()
{
      return worker_list.size();
}  

void CComCloudFileServer::read_data_lock() {
	pthread_mutex_lock(&server_mutex);
};

void CComCloudFileServer::read_data_unlock() {
	pthread_mutex_unlock(&server_mutex);
};


CComCloudFileServer::~CComCloudFileServer()	
{
    printf("Calling the destructor ....\n");
    this->cancel =1;
    pthread_join(thread, NULL);
