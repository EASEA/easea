...
void parseArguments(const char* parametersFileName, int ac, char** av, std::unique_ptr<vm_t> &vm){
	...
	("version,v", "Print version")
        ...
        ("noLogFile", po::bool_switch()->default_value(false), "Disable logging to .log file")
	("alwaysEvaluate", po::bool_switch()->default_value(false), "Always evaluate individual, do not reuse previous fitness")
        ("reevaluateImmigrants", po::bool_switch()->default_value(false), "Set flag if immigrants need to be reevaluated")
        ...
        ("baldwinism", po::bool_switch()->default_value(false), "Only keep fitness")
        ("remoteIslandModel", po::value<int>(), "Boolean to activate the individual exchange with remote islands (default : 0)")
	("silentNetwork", po::bool_switch()->default_value(false), "Do not log informations about sent and received individuals")
...
//  ("printStatsFile",po::value<int>(),"Prints the Stats to a File (Filename: ProjectName.dat) (default : 0)")
        ("printInitialPopulation", po::bool_switch()->default_value("false"), "Print the initial population")
        ("printFinalPopulation", po::bool_switch()->default_value("false"), "Print the final population")
