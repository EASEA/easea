...
void parseArguments(const char* parametersFileName, int ac, char** av, std::unique_ptr<cxxopts::ParseResult> &vm, std::unique_ptr<cxxopts::ParseResult>& vm_file){
	...
	("nbThreads","Set the number of threads", cxxopts::value<int>())
        ...
        ("remoteIslandModel","Boolean to activate the individual exachange with remote islands (default : 0)",cxxopts::value<int>())
        ...
        ("generateCSV","Print the Stats to a CSV File (Filename: ProjectName.dat) (default : 0)",cxxopts::value<int>())
        ("generateCSVFile","Print the Stats to a CSV File (Filename: ProjectName.dat) (default : 0)",cxxopts::value<int>())
        ("generatePlotScript","Generates a Gnuplot script to plat the Stats (Filename: ProjectName.plot) (default : 0)",cxxopts::value<int>())
        ("generateRScript","Generates a R script to plat the Stats (Filename: ProjectName.r) (default : 0)",cxxopts::value<int>())
//  ("printStatsFile",cxxopts::value<int>(),"Print the Stats to a File (Filename: ProjectName.dat) (default : 0)")
        ("printInitialPopulation","Prints the initial population (default : 0)",cxxopts::value<int>())
        ("printFinalPopulation","Prints the final population (default : 0)",cxxopts::value<int>())
        ("savePopulation","Saves population at the end (default : 0)",cxxopts::value<int>())
        ("startFromFile","Loads the population from a .pop file (default : 0",cxxopts::value<int>())
        ("fstgpu","The number of the first GPU used for computation",cxxopts::value<int>())
        ("lstgpu","The number of the fisrt GPU NOT used for computation",cxxopts::value<int>())
