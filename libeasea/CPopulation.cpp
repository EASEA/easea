...
CPopulation::CPopulation(unsigned parentPopulationSize,
...
CPopulation::~CPopulation() {
    for( unsigned i=0 ; i<actualOffspringPopulationSize ;
            i++ ) delete(offsprings[i]);
...
void CPopulation::evaluatePopulation(CIndividual** population,
...
void CPopulation::optimisePopulation(CIndividual** population,
                                     unsigned populationSize) {
}
 ...
 */
void CPopulation::reducePopulation(CIndividual** population,
                                   unsigned populationSize,
...
void CPopulation::sortPopulation(CIndividual** population,
                                 unsigned populationSize) {
    qsort(population,populationSize,sizeof(CIndividual*),CIndividualCompare);
}

void CPopulation::sortRPopulation(CIndividual** population,
                                  unsigned populationSize) {
...
int CPopulation::getWorstIndividualIndex(CIndividual** population) {
    ...
    for(int i=1; i<(signed)this->parentPopulationSize; i++) {
        if((params->minimizing
                && (population[i]->fitness > population[index]->fitness))
                || (!params->minimizing
                    && (population[i]->fitness < population[index]->fitness)))
            index=i;
    }
...
void CPopulation::reduceTotalPopulation(CIndividual** elitPop) {
    ...
    unsigned actualGlobalSize = actualParentPopulationSize
    ...
    if(actualParentPopulationSize==0) {
        memcpy(globalPopulation,offsprings,
               sizeof(CIndividual*)*actualOffspringPopulationSize);
    } else if(actualOffspringPopulationSize==0) {
        memcpy(globalPopulation,parents,
               sizeof(CIndividual*)*actualParentPopulationSize);
    } else {
        memcpy(globalPopulation,parents,
               sizeof(CIndividual*)*actualParentPopulationSize);
    ...
    CPopulation::reducePopulation(globalPopulation,actualGlobalSize,
                                  params->elitSize+nextGeneration,
                                  parentPopulationSize-params->elitSize,replacementOperator,replacementPressure);


    for( unsigned int i=0 ;
            i<((int)actualGlobalSize+params->elitSize)-(int)parentPopulationSize ; i++ )
...
*/
void CPopulation::strongElitism(unsigned elitismSize, CIndividual** population,
                                unsigned populationSize,
...
#ifndef WIN32
    if( elitismSize >= 5 )
        DEBUG_PRT("Warning, elitism has O(n) complexity, elitismSize is maybe too big (%d)",
                  elitismSize);
#endif
...
void CPopulation::weakElitism(unsigned elitismSize,
                              CIndividual** parentsPopulation, CIndividual** offspringPopulation,
                              unsigned* parentPopSize, unsigned* offPopSize, CIndividual** outPopulation,
                              unsigned outPopulationSize) {
    ...
    for(int i = 0 ; (unsigned)i<elitismSize ; i++ ) {
        if(((!params->minimizing && bestParentFitness > bestOffspringFitness)
                || (params->minimizing && bestParentFitness<bestOffspringFitness)
                || (*offPopSize)==0) && (*parentPopSize)>0) {
