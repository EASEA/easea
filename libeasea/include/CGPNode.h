...
class GPNode {
  public:
    ...
    GPNode(int var_id, double erc_value, char opCode,
           GPNode** childrenToAdd) : var_id(var_id), erc_value(erc_value),
        opCode(opCode) { // other constructor
        for(int EASEA_Ndx=0; EASEA_Ndx<2; EASEA_Ndx++)
    ...
    GPNode(const GPNode &EASEA_Var) {  // Copy constructor
        ...
        for(int EASEA_Ndx=0; EASEA_Ndx<2; EASEA_Ndx++)
            if( EASEA_Var.children[EASEA_Ndx] ) children[EASEA_Ndx] = new GPNode(*
                        (EASEA_Var.children[EASEA_Ndx]));
    ...
    bool operator!=(GPNode &EASEA_Var) const {
        return !(*this==EASEA_Var);   // operator!=
    }


    friend ostream& operator<< (ostream& os,
                                const GPNode& EASEA_Var) { // Output stream insertion operator
...

        {
            os << "Array children : ";
            for(int EASEA_Ndx=0; EASEA_Ndx<2; EASEA_Ndx++)
                if( EASEA_Var.children[EASEA_Ndx] ) os << "[" << EASEA_Ndx << "]:" << *
                                                           (EASEA_Var.children[EASEA_Ndx]) << "\t";
        }
...
GPNode* RAMPED_H_H(unsigned iINIT_TREE_DEPTH_MIN, unsigned iINIT_TREE_DEPTH_MAX,
...
GPNode* construction_method( const int constLen, const int totalLen ,
                             const int currentDepth, const int maxDepth, const bool full,
