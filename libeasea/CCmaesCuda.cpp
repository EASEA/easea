...
void CCmaesCuda::Cmaes_init_param(int lambda, int mu) {
    ...
    for (i=0; i<this->mu; ++i)
        this->weights[i] /= s1;
    if(this->mu < 1 || this->mu > this->lambda || (this->mu==this->lambda
            && this->weights[0]==this->weights[this->mu-1])) {
...
//this->stopMaxIter = ceil((float)(this->stopMaxFunEvals / this->lambda));
    ...
    this->damps = this->damps * (1 + 2*MAX(0.,
                                           sqrt((this->mueff-1.)/(this->dim+1.)) - 1)) * 0.3 + this->cs;
...
CCmaesCuda::CCmaesCuda(int lambda, int mu, int problemdim) {
    ...
    this->sigma = sqrt(
                      trace/this->dim); /* this->sp.mueff/(0.2*this->mueff+sqrt(this->dim)) * sqrt(trace/this->dim); */

    this->chiN = sqrt((float) this->dim) * (1. - 1./(4.*this->dim) + 1./
                                            (21.*this->dim*this->dim));
    this->flgEigensysIsUptodate = 1;
...
void CCmaesCuda::Adapt_C2(int hsig, float **parents) {
    ...
    if (this->ccov != 0. && this->flgIniphase == 0) {
        ...
        for (i = 0; i < this->dim; ++i)
            for (j = 0; j <=i; ++j) {
                this->C[i][j] = (1 - this->ccov) * this->C[i][j] + this->ccov *
                                (1./this->mucov) * (this->rgpc[i] * this->rgpc[j] + (1-hsig)*this->ccumcov*
                                                    (2.-this->ccumcov) * this->C[i][j]);
                for (k = 0; k < this->mu; ++k) { /* additional rank mu update */
                    this->C[i][j] += ccovmu * this->weights[k] * (parents[k][i] - this->rgxold[i])
                                     * (parents[k][j] - this->rgxold[j]) / sigmasquare;
                }
            }
...
//  exit(1);
    for (i = 0; i < this->dim; ++i) {
        ...
        this->rgBDz[i] = sqrt(this->mueff)*(this->rgxmean[i] -
                                            this->rgxold[i])/this->sigma;
    }
...
//    }
    ...
    for (i = 0; i < this->dim; ++i) {
        ...
        this->rgps[i] = (1. - this->cs) * this->rgps[i] + sqrt(this->cs *
                        (2. - this->cs)) * sum;
    }
    /* calculate norm(ps)^2 */
    psxps = 0.;
    ...
    hsig = (sqrt(psxps) / sqrt(1. - pow(1.-this->cs,
                                        2*this->gen)) / this->chiN) < (1.4 + 2./(this->dim+1));
    for (i = 0; i < this->dim; ++i) {
        this->rgpc[i] = (1. - this->ccumcov) * this->rgpc[i] + hsig * sqrt(
                            this->ccumcov * (2. - this->ccumcov)) * this->rgBDz[i];
    }
    /* stop initial phase */
    if (this->flgIniphase && this->gen > MIN(1/this->cs, 1+this->dim/this->mucov)) {
        if (psxps / this->damps / (1.-pow((1. - this->cs),
                                          this->gen)) < this->dim * 1.05)
            this->flgIniphase = 0;
    }
