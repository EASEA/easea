...
CEvolutionaryAlgorithm::CEvolutionaryAlgorithm(Parameters* params) {
    ...
    CPopulation::initPopulation(params->selectionOperator,
                                params->replacementOperator,params->parentReductionOperator,
                                params->offspringReductionOperator,
                                params->selectionPressure,params->replacementPressure,
                                params->parentReductionPressure,params->offspringReductionPressure);

    this->population = new CPopulation(params->parentPopulationSize,
                                       params->offspringPopulationSize,
                                       params->pCrossover,params->pMutation,params->pMutationPerGene,
                                       params->randomGenerator,params, this->cstats);
...
void CEvolutionaryAlgorithm::runEvolutionaryLoop() {
    ...
    if(params->elitSize)
        elitistPopulation = (CIndividual**)malloc(params->elitSize*sizeof(
                                CIndividual*));
    ...
    while( this->allCriteria() == false) {
        ...
        if(params->elitSize && this->params->parentPopulationSize>=params->elitSize) {
            ...
            if(params->strongElitism) {
                population->strongElitism(params->elitSize, population->parents,
                                          this->params->parentPopulationSize, elitistPopulation, params->elitSize);
                population->actualParentPopulationSize -= params->elitSize;
            }
            /* WEAK ELITISM */
            else {
                population->weakElitism(params->elitSize, population->parents,
                                        population->offsprings, &(population->actualParentPopulationSize),
                                        &(population->actualOffspringPopulationSize), elitistPopulation,
                                        params->elitSize);
            }
...
void CEvolutionaryAlgorithm::showPopulationStats(struct timeval beginTime) {
    ...
    for(unsigned i=0; i<population->parentPopulationSize; i++) {
        ...
        if( (params->minimizing
                && population->parents[i]->getFitness()<population->Best->getFitness()) ||
                (!params->minimizing
                 && population->parents[i]->getFitness()>population->Best->getFitness()))
        ...
        if( (params->minimizing
                && population->parents[i]->getFitness() > population->Worst->getFitness()) ||
                (!params->minimizing
                 && population->parents[i]->getFitness() < population->Worst->getFitness()))
    ...
    for(unsigned i=0; i<population->parentPopulationSize; i++) {
        this->cstats->currentStdDev+=(population->parents[i]->getFitness()
                                      -this->cstats->currentAverageFitness)*(population->parents[i]->getFitness()
                                              -this->cstats->currentAverageFitness);
    }
    ...
    if(params->printStats) {
        ...
        printf("%u\t%2.6f\t%u\t%.2e\t%.2e\t%.2e\t%.2e\n",currentGeneration,duration,
               population->currentEvaluationNb,population->Best->getFitness(),
               this->cstats->currentAverageFitness,this->cstats->currentStdDev,
               population->Worst->getFitness());
#else
        ...
        printf("%d\t%ld.%d\t\t%d\t\t%.2e  %.2e  %.2e  %.2e\n",(int)currentGeneration,
               res.tv_sec,(int)res.tv_usec/10000,(int)population->currentEvaluationNb,
               population->Best->getFitness(),this->cstats->currentAverageFitness,
               this->cstats->currentStdDev, population->Worst->getFitness());
#endif
    }

    if((this->params->plotStats && this->grapher->valid)
            || this->params->generatePlotScript) {
        FILE *f;
        string fichier = params->outputFilename;
        fichier.append(".dat");
    ...
    if(params->generateCSVFile
            || params->generateRScript) { //Generation du fichier CSV;
        FILE *f;
        string fichier = params->outputFilename;
        fichier.append(".csv");
        f = fopen(fichier.c_str(),"a"); //ajouter .csv
    ...
    if(this->params->plotStats && this->grapher->valid) {
        ...
        fprintf(this->grapher->fWrit,"add coordinate:%d;%f;%f;%f\n",
                population->currentEvaluationNb, population->Best->fitness,
                this->cstats->currentAverageFitness, this->cstats->currentStdDev);
        if(this->params->remoteIslandModel) {
...
void CEvolutionaryAlgorithm::refreshClient() {
    unsigned no_client;
    this->Clients = parse_file(this->params->ipFile,&no_client,
                               this->params->serverPort);

    cout << "ip file : " << this->params->ipFile << " contains " << no_client <<
         " client ip(s)" << endl;
...
void CEvolutionaryAlgorithm::sendIndividual() {
    ...
    if(globalRandomGenerator->random(0.0,1.0)<=params->migrationProbability) {
        ...
        this->population->selectionOperator->initialize(this->population->parents,
                params->selectionPressure, this->population->actualParentPopulationSize);
        ...
        cout << "    Sending my best individual (fitness = " << bBest->getFitness()
             <<") to machine "
             << this->Clients[client]->getIP() << ":" << this->Clients[client]->getPort()
             <<endl;
...
void CEvolutionaryAlgorithm::receiveIndividuals() {
    ...
    if(this->server->parm->data->size() != 0) {
        ...
        CSelectionOperator *antiTournament = getSelectionOperator("Tournament",
                                             !this->params->minimizing, globalRandomGenerator);

        //Treating all the individuals before continuing
        while(this->server->parm->data->size() != 0) {
            //selecting the individual to erase
            antiTournament->initialize(this->population->parents, 7,
                                       this->population->actualParentPopulationSize);
            unsigned index = antiTournament->selectNext(
                                 this->population->actualParentPopulationSize);
...
void CEvolutionaryAlgorithm::generatePlotScript() {
    ...
    fprintf(f,
            "plot \'%s.dat\' using 3:4 t \'Best Fitness\' w lines, \'%s.dat\' using 3:5 t  \'Average\' w lines, \'%s.dat\' using 3:6 t \'StdDev\' w lines\n",
            params->outputFilename,params->outputFilename,params->outputFilename);
    fclose(f);
}
...
void CEvolutionaryAlgorithm::generateRScript() {
    ...
    fprintf(f,
            "plot(0, type = \"n\", main = \"Plot Title\", xlab = \"Number of Evaluations\", ylab = \"Fitness\", xlim = c(0,%d) )\n",
            (int)population->currentEvaluationNb);
